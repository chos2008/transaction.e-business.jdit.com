...
<script type="text/javascript" src="../js/sheet-order.js"></script>
</head>
<body>
...
<c:choose>
	<c:when test="${empty orders}">
		<div class="item-not-issue" style="height: 160px; margin: 2px 5px 3px; padding: 3px 5px; border: 0px solid silver; position: relative; background-color: white; background-image: url('../images/qapp_center_loading.png'); background-position: center center; background-repeat: no-repeat; opacity: 0.6;">
			<p style="width: 100%; margin: 0px 0px; padding: 0px 0px; position: absolute; bottom: 0px; text-align: center;">
				<a style="font-size: 12px; font-family: Arial, 'Microsoft YaHei';" href="issue.jsp">你还没有单子~</a>
			</p>
		</div>
	</c:when>
		...
		<li class="list-item-t-item-normal" style="width: 100%; padding: 14px 0px;">
			<c:choose>
				<c:when test="${empty userAddress}">
					<p style="height: 32px; line-height: 32px; padding: 0px 5px;"><a href="../address/add/view.shtml" target="_blank">添加收货地址</a></p>
				</c:when>
				<c:otherwise>
					<p style="height: 32px; line-height: 20px; padding: 0px 5px;">
						收货人：${userAddress.username}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${userAddress.contact}<br>
						收货地址：${userAddress.address}<c:if test="${not empty userAddress.postCode}">(${userAddress.postCode})</c:if>
					</p>
				</c:otherwise>
			</c:choose>
		</li>
	...
	<ul class="list-item" style="background-color: silver;">
		...
		<c:set value="${amount + variable.quantity * variable.amount}" var="amount" />
		<li class="list-item-t-item-default order-sheet-item">
			...
			<c:choose>
				...
				<ul class="list-item">
					<li class="list-item-t-item-right" style="font-family: Arial;"><p>共${variable.quantity}件 实付：&yen;${variable.quantity * variable.amount}</p></li>
				</ul>
	...
	<ul class="list-item" style="height: 60px; border-bottom-width: 0px;">
		<li class="list-item-t-item-normal" style="padding: 30px 8px;">
			<!-- 
			<p style="height: 32px; line-height: 32px;">共${fn:length(orders)}笔交易 总金额：&yen;${amount}</p>
			 -->
		</li>
		<li class="list-item-t-item-normal-right" style="padding: 30px 8px; font-family: Arial;">
			<a class="go-trade" href="javascript: void(0);" style="width: 70px; margin: 0px auto; height: 32px; line-height: 32px; display: block; color: #fff; background-color: #ff3c3c; border: 1px solid #BE4646; border-radius: 3px; -webkit-border-radius: 3px; text-align: center; font-size: 0.5rem;">提交订单</a>
		</li>
	
		<li class="list-item-t-item-normal-right" style="padding: 30px 0px; font-family: Arial;">
			<p style="height: 32px; line-height: 32px;">合计：&yen;${amount}</p>
		</li>
	</ul>
	</c:otherwise>
</c:choose>
